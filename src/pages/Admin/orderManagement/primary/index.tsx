...
export default function PrimaryOrder() {
  ...
  const handleView = (rowItem: Record<string, any>) => {
    let pathUrl = generatePath(
      APP_ROUTES?.ADMIN?.ORDER_MANAGEMENT?.PRIMARY?.view,
      {
        id: rowItem?.id,
      }
    );
  ...
  const getOrderCount = async () => {
    try {
      const payload = {
        ...
        type: user?.CONSTANT_DATA?.ORDER_SETTING_TYPES[0],
      };
  ...
  const getStatusList = async () => {
    try {
      const payload = {
        matchObj: {
          status: true,
          settingsType: user?.CONSTANT_DATA?.ORDER_SETTING_TYPES[0],
        },
        organization_id: user.organization_id,
      };
