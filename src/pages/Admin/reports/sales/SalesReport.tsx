...
import ReportService from '../../../../services/admin/reports/report.service.tsx';
...
const SalesReport = () => {
    ...
    const fetchColumns = async () => {
      try {
        const response = await ReportService?.getColumnsClaims(
          '?report_name=SALES_REPORT_COLUMN'
        );
    ...
    const handleDownload = async () => {
        try {
          ...
          const response = await ReportService.salesReport(payload);
          const filename = `sales_report${moment().format('DD-MM-YYYY')}.csv`;
    
          FileDownload(response.data, filename);
        } catch (error: any) {
          console.error('Error downloading report:', error);
          if (error?.response?.status == 500) {
            toast.warning('Sales report data not found', {
