...
import groupSettingService from '../../../../../src/services/settings/group.setting.service.ts';
...
const VisitReports = () => {
  ...
  const fetchColumns = async () => {
    try {
      const response = await AttendanceReport?.getColumnsAttendance(
        '?report_name=ADMIN_VISIT_REPORT'
      );
  ...
  const handleDownload = async () => {
    try {
      ...
      const response = await AttendanceReport.DownloadVisit(payload);

      const filename = `visit_report${moment().format('DD-MM-YYYY')}.csv`;

      FileDownload(response.data, filename);
    } catch (error: any) {
      console.error('Error downloading report:', error);
      if (error?.response?.status == 500) {
        toast.warning('Visit report data not found', {
