...
const LeaveReports = () => {
  ...
  const handleDownload = async () => {
    try {
      ...
      const response = await AttendanceReport.DownloadLeave(payload);

      const filename = `leave_report${moment().format('DD-MM-YYYY')}.csv`;

      FileDownload(response.data, filename);
    } catch (error: any) {
      console.error('Error downloading report:', error);
      if (error?.response?.status == 500) {
        toast.warning('Leave report data not found', {
