...
// ************** | Form schema
import { PriorityLeadSchema } from "../../../../../data/yup/settings/settings.ts";
...
function AddEditViewModal(props: IAddEditViewModal) {
  ...
  const ResetForm = () => {
    formik.resetForm();
    setFormValue({ leadPriority: null, status: true });
  };
  ...
  const handleCreatePriorityType = async () => {
    try {
      ...
      if (isEdit) {
        console.log("Editing existing priority type:", {
          ...priorityTypeData,
          id: id,
        });
        response = await SettingService.editLeadPriorityTypes({
          ...priorityTypeData,
        });
      } else {
        console.log("Creating new priority type:", priorityTypeData);
        response = await SettingService.createLeadPriorityTypes(
          priorityTypeData
        );
      }
      ...
      if (response && response.id) {
        console.log("priority type saved successfully:", response);
        refresh && refresh();
        handleClose && handleClose();
      } else {
        formik.resetForm();
        console.error("Failed to save priority type:", response);
        handleClose && handleClose();
        refresh && refresh();
      }
    } catch (error) {
  ...
  const formik = useFormik({
    validationSchema: PriorityLeadSchema({}),
            ...
            <Grid item xs={12}>
              <Box>
                ...
                <Textfield
                  ...
                  fullWidth
                  error={getError(formik, "leadPriority")?.isTrue}
                  helperText={
                    getError(formik, "leadPriority")?.isTrue &&
                    getError(formik, "leadPriority")?.message
                  }
...
function priorityType() {
  ...
  const tab = [
    {
      label: "Active Priority Type",
      icon: <CheckCircle fontSize="small" />,
      status: true,
    },
    {
      label: "De-Active Priority Type",
  ...
  const getList = async () => {
    ...
    setLoading(true);
    try {
      const listRes = await SettingService.listLeadPriority(payload);
  ...
  const columnDefs: ColDef[] = useMemo(
    () => [
      ...
      // },
      {
        ...
        cellRendererParams: { field: "leadPriority" },
        ...GetHeaderParams(),
      },
      {
        headerName: "Actions",
