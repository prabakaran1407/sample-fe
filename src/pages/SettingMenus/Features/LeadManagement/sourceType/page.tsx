...
// ************** | Form schema
import { SourceLeadSchema } from "../../../../../data/yup/settings/settings.ts";
...
function AddEditViewModal(props: IAddEditViewModal) {
  ...
  const ResetForm = () => {
    formik.resetForm();
    setFormValue({ leadSource: null, status: true });
  };

  const handleCreateSourceType = async () => {
    try {
      ...
      if (isEdit) {
        ...
        response = await SettingService.editLeadSourceTypes({
          ...sourceTypeData,
        });
      } else {
        console.log("Creating new task type:", sourceTypeData);
        response = await SettingService.createLeadSourceTypes(sourceTypeData);
      }
  ...
  const formik = useFormik({
    validationSchema: SourceLeadSchema({}),
    onSubmit: handleCreateSourceType,
            ...
            <Grid item xs={12}>
              <Box>
                ...
                <Textfield
                  ...
                  fullWidth
                  error={getError(formik, "leadSource")?.isTrue}
                  helperText={
                    getError(formik, "leadSource")?.isTrue &&
                    getError(formik, "leadSource")?.message
                  }
...
function sourceType() {
  ...
  const tab = [
    {
      label: "Active Source Type",
      icon: <CheckCircle fontSize="small" />,
      status: true,
    },
    {
      label: "De-Active Source Type",
  ...
  const getList = async () => {
    ...
    try {
      const listRes = await SettingService.listLeadSource(payload);
  ...
  const columnDefs: ColDef[] = useMemo(
    () => [
      ...
      // },
      {
        ...
        cellRendererParams: { field: "leadSource" },
        ...GetHeaderParams(),
      },
      {
        headerName: "Actions",
