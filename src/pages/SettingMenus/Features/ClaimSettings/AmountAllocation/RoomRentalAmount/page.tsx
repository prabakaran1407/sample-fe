...
const validationSchema = yup.object({
  ...
  amount: yup
    .string()
    .trim()
    .required("Cost of room is required.")
    .notOneOf([" "], "Cost of room should not contain spaces."),
});
  ...
  const handleSubmit = async (value: Record<string, any>) => {
    try {
      let data: any = {
        ...
        costOfRoom: value?.amount,
        status: value?.checked,
        organization_id: AUTH?.data?.userRecord?.organization_id,
      } as Ipayload;

      const response = await AmountAllocationService.getRoomRentList({
        status: value?.checked,
        organization_id: AUTH?.data?.userRecord?.organization_id,
      });
      ...
      if (isEdit) {
        ...
        if (!temp) {
          ...
          formik.setFieldError("amount", "Cost of room already allocated.");
        }
      }
    } catch (error: any) {
      if (!error?.message) {
    ...
    <ActionModal
      ...
      title={
        actionType === ACTION_ICON_TYPES[0]
          ? "New Room Rent Allocation"
          : actionType === ACTION_ICON_TYPES[1]
          ? "Edit Room Rent Allocation"
          : "View Room Rent Allocation"
      }
    >
      <Box
        sx={{
            ...
            <Grid item xs={6}>
              <Box>
                <InputLabel
                  ...
                  sx={{ fontSize: 18, fontWeight: "600", color: "#181C32" }}
                >
                  Cost Of Room <span style={{ color: "red" }}>*</span>
                </InputLabel>
                <Textfield
                  type="number"
                  name="amount"
...
// ********************* | Main List Page | *****************
function RoomRentalAmountAllocation() {
  ...
  const handleDelete = (value: TNestedObj) => {
    ...
    setOpenConfirmation({
      ...
      message: `Are you sure you want to deactivate this room rental amount allocation?`,
    });
  };
  ...
  const handleActivate = (value: Record<string, any>) => {
    ...
    setOpenConfirmation({
      ...
      message: `Are you sure you want to activate this room rental amount allocation?`,
    });
  };
  ...
  const getList = async () => {
    ...
    const response = await AmountAllocationService.getRoomRentList(payload);
...
export default memo(RoomRentalAmountAllocation);
