...
const validationSchema = yup.object({
  name: yup
    ...
    .notOneOf([" "], "Claim type should not contain spaces."),
});
...
function AddEditViewModal(props: IAddEditViewModal) {
  ...
  const handleSubmit = async (value: Record<string, any>) => {
    try {
      let data: any = {
        typeOfClaim: value?.name?.toUpperCase(),
        status: value?.checked,
        organization_id: AUTH?.data?.userRecord?.organization_id,
      } as Ipayload;
      const response = await ClaimSettingManagementService.getClaimTypes(data);
      ...
      if (isEdit) {
        if (!temp) {
          ...
          formik.setFieldError("name", "Claim type already exists.");
        }
      } else {
        if (!temp) {
...
function ClaimType() {
  ...
  const handleDelete = (value: TNestedObj) => {
    ...
    setOpenConfirmation({
      open: true,
      title: "Deactivate Type Of Claim",
      message: `Are you sure you want to deactivate this type of claim?`,
    });
  };
  ...
  const handleActivate = (value: Record<string, any>) => {
    ...
    setOpenConfirmation({
      open: true,
      title: "Activate Type Of Claim",
      message: `Are you sure you want to activate this type of claim?`,
    });
  };
  ...
  const getList = async () => {
    ...
    const response = await ClaimSettingManagementService.getAllClaimTypes(
      payload
    );
  ...
  const TABS = useMemo(
    () => [
      {
        label: "Active Claim Type",
        icon: <CheckCircle fontSize="small" />,
      },
      {
        label: "Deactivated Claim Type",
