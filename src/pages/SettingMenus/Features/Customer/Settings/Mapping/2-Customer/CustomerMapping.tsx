...
// import CustomerSettingService from '../../../../../../../services/settings/customer.setting.service.ts';
...
function CustomerMapping({ selectedOption, AUTH }: IAddEditViewHierarchy) {
  //  ***************** States
  ...
  const getAssignUnAssignList = async () => {
    ...
    try {
      let payload = {
        ...
        status: true,
        group: filterOption?.group,
      };
      let query = `?assign_or_unassign=${
        activeTab === 0 ? 'FOR_ASSIGN' : 'FOR_UNASSIGN'
      }`;
      let listData = await UCBMappingService.listAssignUnAssignCustomer(
        payload,
        query
      );
  ...
  const getInitialData = async () => {
    try {
      ...
      // let customerData = await CustomerSettingService.getAll(payload);
      let groupRes = await GroupSettingService.getAll(payload);
      groupRes = groupRes?.data;
      setAssigneeData(
        groupRes?.data?.map((m: any) => ({
          label: m?.groupName,
          value: m?.id,
        })) || []
      );
      console.log('constgroupRes >>>>>>>', groupRes);
      // customerData = customerData?.data;
      // console.log('customerData >>>>>>>', customerData);
      // if (customerData?.status) {
      //   setAssigneeData(
      //     customerData?.data?.map((m: Record<string, any>) => ({
    ...
    setOpenConfirmation({
      ...
      title:
        selectedMenu === ASSIGN_OR_UNASSIGN[0]?.value
          ? 'Assign Customer Mapping'
          : 'Un-Assign Customer Mapping',
      message:
        selectedMenu === ASSIGN_OR_UNASSIGN[0]?.value
  ...
  async function handleSubmit() {
    try {
      if (selectedMenu === ASSIGN_OR_UNASSIGN[0]?.value) {
        let payload = {
          group: assignedUser?.value,
          organization_id: AUTH?.data?.userRecord?.organization_id,
          // createdBy: AUTH?.data?.userRecord?.id,
          // updatedBy: AUTH?.data?.userRecord?.id,
        } as Record<string, any>;
        payload.customers = selectedRow?.map((items: any) => items?.id);
        let assignResponse =
          await UCBMappingService.createAssignUnAssignCustomerMapping(payload);
        ...
        payload.customers = selectedRow?.map((items: any) => items?.id);
        let unAssignRes =
          await UCBMappingService.updateAssignUnAssignCustomerMapping(payload);
