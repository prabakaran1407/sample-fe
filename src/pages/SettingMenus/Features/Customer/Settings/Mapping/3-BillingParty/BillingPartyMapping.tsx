...
function BillingPartyMapping({ selectedOption, AUTH }: IAddEditViewHierarchy) {
  ...
  const getAssignUnAssignList = async () => {
    ...
    try {
      ...
      let listData = await UCBMappingService.listAssignUnAssignBillingParty(
        payload,
        query
      );
  ...
  async function handleSubmit() {
    try {
      if (selectedMenu === ASSIGN_OR_UNASSIGN[0]?.value) {
        ...
        payload.billingParties = selectedRow?.map((items: any) => items?.id);
        let assignResponse =
          await UCBMappingService.createAssignUnAssignBillingPartyMapping(
            payload
          );
        ...
        payload.billingParties = selectedRow?.map((items: any) => items?.id);
        let unAssignRes =
          await UCBMappingService.updateAssignUnAssignBillingPartyMapping(
            payload
          );
...
export default BillingPartyMapping;
