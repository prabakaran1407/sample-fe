...
// ************** | Form schema
import { productTypeSchema } from "../../../../../data/yup/settings/settings.ts";
...
function AddEditViewModal(props: IAddEditViewModal) {
  ...
  const handleSubmit = async (value: Record<string, any>) => {
    try {
      ...
      let query = `?setting_type=${CONSTANT_DATA?.PRODUCT_SETTING_TYPES[0]}&sub_type=${CONSTANT_DATA?.PRODUCT_SETTING_SUB_TYPES[3]}`;
      if (isEdit) {
        payload = {
          ...payload,
      ...
      if (!error?.response?.data?.status) {
        if(error?.response?.data?.response?.isDuplicate){
          formik.setFieldError('producttype', error?.response?.data?.message)
          return 
        }
            ...
            <Grid item xs={6}>
              <Box>
                ...
                <Textfield
                  ...
                  type="text"
                  name="producttype"
                  value={(formik?.values?.producttype || "").trimStart()}
                ...
                >
                  <ButtonV1
                    disabled={
                      ...
                      !formik.values.producttype.trim()
                    }
                    type="submit"
...
function ProductType() {
  ...
  const handleDelete = (value: TNestedObj) => {
    ...
    setOpenConfirmation({
      ...
      message: `Are you sure you want to deactivate this product type?`,
    });
  };
  ...
  const handleActivate = (value: Record<string, any>) => {
    ...
    setOpenConfirmation({
      open: true,
      title: "Activate Product Type",
      message: `Are you sure you want to activate this product type?`,
    });
  };
  ...
  const getList = async () => {
    ...
    let query = `?setting_type=${CONSTANT_DATA?.PRODUCT_SETTING_TYPES[0]}&sub_type=${CONSTANT_DATA?.PRODUCT_SETTING_SUB_TYPES[3]}&isCount=true&listType=LIST`;
  ...
  const getListFilter = async () => {
    ...
    let query = `?setting_type=${CONSTANT_DATA?.PRODUCT_SETTING_TYPES[0]}&sub_type=${CONSTANT_DATA?.PRODUCT_SETTING_SUB_TYPES[3]}&isCount=true&listType=DROPDOWN_LIST`;
      ...
      <div>
        ...
        >
          <ProductSettingBulkUpload
            ...
            sub_type={CONSTANT_DATA?.PRODUCT_SETTING_SUB_TYPES[3]}
          />
        </MyDrawer>
        <ActionConfirmation
