...
const validationSchema = yup.object().shape({
  primaryStatus: yup.string().trim().required("Primary status is required"),
});
...
function AddEditViewModal(props: IAddEditViewModal) {
  ...
  const handleSubmit = async (value: Record<string, any>) => {
    const handleError = (error: any) => {
      ...
      if (errorMessage.includes("type")) {
        ...
        formik.setFieldError("primaryStatus", errorMessage);
      }
    };
    try {
      let payload = {
        value: value?.primaryStatus,
        type: value?.type,
        description: value?.description,
        organization_id: AUTH?.data?.userRecord?.organization_id,
        settingsType: CONSTANT_DATA.ORDER_SETTING_TYPES[0],
  ...
  const ResetForm = () => {
    formik.resetForm();
    setFormValue({ primaryStatus: null, type: "Others", description: null });
  };
              ...
              <Grid container>
                <Grid item xs={12}>
                  <Box sx={{ py: 1, width: "80%" }}>
                    ...
                    >
                      Primary Status <span style={{ color: "red" }}>*</span>
                    </InputLabel>
                    <Textfield
                      type="text"
                      name="primaryStatus"
                      value={(formik?.values?.primaryStatus || "").trimStart()}
                      fullWidth
                      error={getError(formik, "primaryStatus")?.isTrue}
                      helperText={
                        getError(formik, "primaryStatus")?.isTrue &&
                        getError(formik, "primaryStatus")?.message
                      }
                ...
                >
                  <ButtonV1
                    disabled={
                      formik?.isSubmitting ||
                      !formik.isValid ||
                      !formik.values.primaryStatus?.trim() ||
...
function PrimaryStatus() {
  ...
  const handleDelete = (value: Record<string, any>) => {
    ...
    setOpenConfirmation({
      open: true,
      title: "Deactivate Primary Status",
...
export default memo(PrimaryStatus);
