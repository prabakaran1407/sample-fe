...
// ************** | Form schema
import { PrioritySchema } from "../../../../../data/yup/settings/settings.ts";
...
function AddEditViewModal(props: IAddEditViewModal) {
  ...
  const handleCreatePriorityType = async () => {
    try {
      const organizationId = AUTH?.data?.userRecord?.organization_id;
      const priorityTypeData = {
        priority: formik.values.priorityType,
        status: formik.values.status,
        organization_id: organizationId,
      };

      if (isEdit) {
        // Check if the updated priority type already exists
        const settingList = await SettingService.listPriority({
          organization_id: organizationId,
        });

        const temp = settingList?.data?.data?.find(
          (item: any) =>
            item.priority === priorityTypeData.priority &&
            item.organization_id === priorityTypeData.organization_id &&
            item.id !== id // Exclude the current priority type from the check during edit
        );

        if (!temp) {
          // Update existing priority type
          await SettingService.editPriorityTypes({
            ...priorityTypeData,
            id: id,
          });
          ResetForm();
          ...
          formik.setFieldError("priorityType", "Priority type already exists.");
          // Additional handling if needed
        }
      } else {
        // Create new priority type
        const settingList = await SettingService.listPriority(priorityTypeData);
        const temp = settingList?.data?.data?.find(
          (item: any) =>
            item.priority === priorityTypeData.priority &&
            item.organization_id === priorityTypeData.organization_id
        ...
        if (!temp) {
          await SettingService.createPriorityTypes(priorityTypeData);
          ResetForm();
          refresh && refresh();
          handleClose && handleClose();
        } else {
  ...
  const formik = useFormik({
    ...
    onSubmit: handleCreatePriorityType,
  ...
  const ResetForm = () => {
    formik.resetForm();
    setFormValue({ priorityType: null, status: true });
  };
          ...
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <Box sx={{ py: 1 }}>
                ...
                >
                  Priority type <span style={{ color: "red" }}>*</span>
                </InputLabel>
                <Textfield
                  type="text"
                  name="priorityType"
                  value={(formik?.values?.priorityType || "").trimStart()}
                  fullWidth
                  error={getError(formik, "priorityType")?.isTrue}
                  helperText={
                    getError(formik, "priorityType")?.isTrue &&
                    getError(formik, "priorityType")?.message
                  }
                  onChange={formik?.handleChange}
                  onBlur={formik?.handleBlur}
                  placeholder="Enter priority type"
                  disabled={isView}
                />
              </Box>
                ...
                >
                  <ButtonV1
                    disabled={
                      formik?.isSubmitting ||
                      !formik.isValid ||
                      !formik.values.priorityType?.trim()
                    }
                    type="submit"
...
function priority() {
  ...
  const handleDelete = (value: Record<string, any>) => {
    ...
    setOpenConfirmation({
      ...
      message: `Are you sure you want to deactivate this priority?`,
    });
  };
  ...
  const handleActivate = (value: Record<string, any>) => {
    ...
    setOpenConfirmation({
      ...
      message: `Are you sure you want to activate this priority?`,
    });
  };
  ...
  const getList = async () => {
    ...
    setLoading(true);
    try {
      const listRes = await SettingService.listPriority(payload);
      if (listRes?.status) {
        setTableData(listRes?.data?.data || []);
        setTotalCount(listRes?.data?.total);
      } else {
        console.log("Priority type response failed");
      }
    } catch (error) {
